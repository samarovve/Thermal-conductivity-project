...
class SuperCubeStatic:
    ...
    def __init__(self, size: int) -> None:
        ...
        if size >= 7:
            ...
            rows: List[int] = [0]
            columns: List[int] = []
            for z, y, x in product(range(1, self.size - 1), repeat=3):
                members = 0
                if z > 1:
                    data.append(-1.0)
                    members += 1
    ...
    def get_temp_from_cords(
        ...
        Если точка принадлежит нескольким кубам (на границе),
        возвращается среднее их температур.
        ...
        for ix, iy, iz in product(range(self.size), repeat=3):
